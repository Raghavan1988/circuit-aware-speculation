...
\begin{abstract}
A large model writes one token at a time, and that is slow. Each new token needs
a full pass through the model. Speculative decoding (SD) is one way to make it
faster. A small drafter model guesses the next few tokens. The large target
model then checks them all in one pass. Guesses that match what the target would
have written are kept. Each kept guess saves a pass. The rest are thrown out.
The compute that made them is lost. So it pays to know, up front, when a round
is worth drafting.

In SD, every control signal in deployed use today is worked out \emph{after} the
draft tokens exist: draft model entropy, agreement heads on draft hidden states,
or acceptance history. By then you have paid for the draft. That is the wrong end
of the round.

We show that the \emph{target} model already holds the signal, one round ahead.
We call it the frontier state: the target's own cached residual stream state at
the last checked token. It is in memory when the round starts. It comes free
from the check that just ran. A linear probe on the frontier state predicts
whether the \emph{next} round's first drafted token will be accepted,
\emph{before any draft compute is spent}.

We score the probe against a hardened pre-round baseline. That baseline uses
target entropy from the round before, top-1/top-2 margin, recent acceptance
history, and domain. On top of it, the probe adds $+0.05$--$+0.10$ AUROC. The
gain is replicated across two model families (Qwen2.5-7B/0.5B,
Llama-3.1-8B/3.2-1B) and two corpora (4-domain and 7-axis). All of these numbers
come from frozen test splits under two protocols. One of them is a strict
dev$\to$test transfer in which no parameter, scaler, or calibrator ever touches
test data.

Controls back this up. Random controls with equal capacity collapse to gains at
chance level. Platt calibration leaves probabilities you can act on (test ECE
$0.016$--$0.037$). The probe costs ${\sim}16\,\mu s$ per round, or
${\sim}0.015\%$ of a decode round.

Steering along the probe direction disrupts realized acceptance $2$--$10\times$
more than random and shuffled controls matched for norm. The effect grows with
dose. It holds beyond the entropy change that steering brings with it. It shows
up in both model families. That is evidence that the state is causally
load-bearing for acceptance, not a side effect.

The signal is \emph{first-token only}. Survival probes run per length show no
usable lift for run length ($k\ge4$). That draws a clean line between this
result and draft length prediction.

In a separate line of work, we built an atlas of how acceptance behaves. We ran
it as a frozen test against pre-registered criteria. In it, acceptance is
strongly shaped by token category. Structural tokens gain $+0.12$ to $+0.21$
over entity tokens, and are CI-clean in all $15$ domain cells. A phase axis
based on absolute position does not survive.

We frame the finding as an honest diagnostic. It adds to draft entropy control
within the round rather than replacing it. Its decision value is strongest where
drafts are costly.
\end{abstract}
...
This section defines each term the paper uses. Read it once and the rest of
the paper should need no glossary.
