...
- **Follow-on (I26, 2026-09-06):** T3.4 showed neither compile mode helps on the *dynamic*-cache decode path, so the deployed-regime draft cost stayed unmeasured. I26 adds the fixed-shape **`StaticCache`** step T3.4 named as the fix — `src/cas/static_decode.py` + `modal_app.py::bench_static_draft` — as a latency-characterization path under this decision's scope. Forward-step token-identity is CPU-verified (`tests/test_static_cache_equiv.py`); a correct static-cache **rollback** is a separate, version-sensitive KV-mask problem (measured to leak stale K/V on transformers 5.13, recorded as an xfail), which keeps a *lossless* static-generate path in Tier-2/G4, not here. See CLAIMS_LEDGER Run log 2026-09-06.
