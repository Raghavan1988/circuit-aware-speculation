...
| **EAGLE-3 (arXiv:2503.01840)** | drafter (trained head) | one-layer draft head over **fused low/mid/high target hidden states**; direct-token "training-time test" objective; reuses frozen target embed+LM head | descriptive only (low=syntax/mid=relations/high=semantics; **no probing/causal analysis of acceptance**) | **verified (abs/HTML 2026-07-24)** | **I25 substrate, not a core-manuscript pre-emption.** Trains on target features — the mechanism I25/Q2 tests our pre-round probe against. **Q3 opening:** what the fused features encode about acceptance is unstudied. No official Qwen2.5-7B head (community: `ruipeterpan/...EAGLE3_UltraChat`, `thoughtworks/...Eagle3`); official heads = Vicuna-13B, LLaMA-3.1-8B, LLaMA-3.3-70B, DeepSeek-R1-Distill-8B (HF `yuhuili`) | I25 drafter; cite |
| **TAPS (arXiv:2603.27027)** | drafter pool (per-domain trained) | per-domain EAGLE-2/HASS drafters + **confidence-based routing** + merged-tree verify | no | **verified (abs/HTML 2026-07-24; full PDF read pending before I25 compute)** | **PRE-EMPTS I25/Q1** (trained drafts revive domain specialization; routing beats single-domain draft) and independently reports **confidence > entropy** as routing signal. Does NOT use a target-side pre-round probe, no oracle-router upper bound. Q1 becomes replication-of-TAPS, cited; does not touch C10/C04 core claims | cite (I25 Q1); read full |
